...
class App extends React.Component {
  ...
  constructor(props) {
    ...
    this.AddbookModal = React.createRef();
    this.chatDrawer = React.createRef();
  ...
  handleAddBook = () => {
    this.AddbookModal.current.handleModalShowHide();
  }


  OpenChatWith = (username) => {
    // this.chatComponent.current.OpenChatWith(username)
    this.chatDrawer.current.OpenChatWith();
  }
  ...
  render() {
    ...
    return (
      <Router>
        ...
        <SignUp openLogin={() => { this.LoginModal.current.handleModalShowHide() }} submit={this.submitt} ref={this.regModal} />
        <AddBook dir="rtl" serverAddress={this.serverAdress} ref={this.AddbookModal} />
        <Chat dir="rtl" serverAddress={this.serverAdress} ref={this.chatDrawer} />
        <Switch ref={this.bookCards}>
          <Route exact path="/">
            ...
            <FilterIcon onChange={(fil) => { this.filter(fil) }} />
            <AppProvider ref={this.searchresultviewer}>
              <BookList onChat={(username) => { this.OpenChatWith(username) }} serverAddress={this.serverAddress} ref={this.bookCards} results={this.state.results} reload={this.state.results_reload} filter={this.state.filter} />
            </AppProvider>

          </Route>

          <Route exact path="/email-verified-successfully">
            <br /><br /><br />
            <p>حساب شما تائید شد</p>
